"""
exportar_resultados_graficos.py — gera os 3 CSVs canônicos que o gerar_graficos.py
consome (e que hoje fazem fig3a/fig3b/fig5b serem puladas).

Reaproveita SUAS funções (evaluate.py e recomendador.py), então os números das
figuras são idênticos aos do artigo — sem digitar nada à mão.

Saída (em OUTPUTS_TABLES/'resultados_gerar_grafico'):
  - backtest_modelos.csv      -> fig3a  (modelo, mape_medio, mape_std)
  - erro_por_trimestre.csv    -> fig3b  (trimestre, baseline, lightgbm)
  - exemplos_recomendador.csv -> fig5b  (rotulo, p10, p50, p90, pedido, situacao)

Coloque este arquivo em src/modeling/ e rode ANTES do gerar_graficos.py:
    python src/modeling/exportar_resultados_graficos.py
"""
