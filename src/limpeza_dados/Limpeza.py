...
# =============================================================================
#  ITEM 9 — Filtro de subdeclaração CIENTE DE REGIME (substitui o antigo
#  filtrar_subdeclaracao_por_classe). No regime antigo a base era a avaliação
#  da PBH e declarar baixo não mexia nela -> piso por classe faz sentido. No
#  regime novo a base É o declarado -> aplicar o piso removeria o próprio alvo;
#  então só cortamos erro grosseiro.
# =============================================================================
def filtrar_subdeclaracao_por_regime(df, piso_erro_grosseiro=None):
    if piso_erro_grosseiro is None:
        piso_erro_grosseiro = PISO_PRECO_M2_ERRO_GROSSEIRO
    print(f"\n[7.6/8] Filtrando subdeclaração (ciente de regime)...")
    print(f"  Linhas antes: {len(df):,}")

    regime = classificar_regime(df['data_transacao'])
    pre = np.isin(regime, ['antigo', 'transicao'])   # regime antigo/transição
    pos = regime == 'novo'

    classe = df['bairro'].map(BAIRRO_PARA_CLASSE).fillna(GRUPO_BAIRRO_PADRAO)
    piso_classe = classe.map(PISO_PRECO_M2_POR_CLASSE)

    mask_ok = pd.Series(True, index=df.index)
    # Regime antigo/transição: piso por classe (rede contra subdeclaração).
    mask_ok[pre] = df.loc[pre, 'preco_m2'] >= piso_classe[pre]
    # Regime novo: só erro grosseiro (o declarado baixo é base legal legítima).
    mask_ok[pos] = df.loc[pos, 'preco_m2'] >= piso_erro_grosseiro

    rem_pre = int((~mask_ok[pre]).sum())
    rem_pos = int((~mask_ok[pos]).sum())
    print(f"  ✓ Removidos regime antigo/transição (piso por classe): {rem_pre:,}")
    print(f"  ✓ Removidos regime novo (erro < R$ {piso_erro_grosseiro}/m²): {rem_pos:,}")
...
def aplicar_filtros_rigorosos(df):
    ...
    # FILTRO 0: residenciais; só casas (CA) e apartamentos (AP)
    mask_residencial = df['tipo_ocupacao'] == 'RESIDENCIAL'
    removidos_residencial = (~mask_residencial).sum()
    df = df[mask_residencial].copy()
    print(f"Filtro residencial: {removidos_residencial:,} removidos (não-residenciais)")
    ...
    print(f"Filtro tipo habitacional: {removidos_tipo:,} removidos (mantidos AP e CA)")
    ...
    # FILTRO 7: subdeclaração CIENTE DE REGIME (item 9)
    df = filtrar_subdeclaracao_por_regime(df)
...
def filtrar_outliers_preco_m2_por_bairro(df, anos_treino=None,
                                         k_iqr_inf=1.5, k_iqr_sup=3.0,
                                         min_transacoes=30):
    ...
    if anos_treino is not None:
        ...
        print(f"Estatísticas sobre {len(df_base):,} linhas de treino "
              f"(anos {sorted(anos_treino)})")
    else:
        df_base = df
        print(f"Estatísticas sobre todo o dataset (possível leakage leve)")
    ...
    stats = pd.DataFrame({'n': g.count(), 'q1': quartis['q1'], 'q3': quartis['q3']})
    ...
    print(f"Grupos (bairro+tipo) confiáveis (>= {min_transacoes}): "
          f"{n_conf} de {len(stats)}")
    ...
    print(f"Outliers removidos: {removidos:,} ({removidos / len(df) * 100:.2f}%)")
...
def deflacionar_valores(df):
    ...
    if meses_sem_fator:
        raise ValueError(
            ...
            f"Atualize IPCA_VAR_MENSAL no config.py com esses meses."
        )
    ...
    print(f"Fatores aplicados — de {f_min:.3f}x (recentes) a {f_max:.3f}x (antigas)")
    med_nom = df['valor_base_calculo'].median()
    med_real = df['valor_base_calculo_real'].median()
    print(f"Mediana base de cálculo: R$ {med_nom:,.0f} (nominal) -> "
...
def limpar_dataset_itbi(df=None, path_input=None):
    ...
    df_clean = df_clean.dropna(subset=['valor_base_calculo_real',
                                       'area_construida_m2',
    ...
    print(f"Período: {df_clean['ano_transacao'].min():.0f} - "
          f"{df_clean['ano_transacao'].max():.0f}")
    print(f"Valor médio (base cálc., dez/2024): R$ "
          f"{df_clean['valor_base_calculo_real'].mean():,.2f}")
